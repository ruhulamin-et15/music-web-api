...
const changePasswordIntoDB = async (
  ...
  const previousPasswords: PasswordHistory[] = await PasswordHistoryModel.find({
    userId: user._id,
  })
    .sort({ timestamp: -1 })
    .limit(2);
  ...
  const isPasswordRepeated = previousPasswords.some((prevPassword) => {
    const isMatch = bcrypt.compareSync(
      payload.newPassword,
      prevPassword.passwordHash
    );
    if (isMatch) {
      const formattedTimestamp = prevPassword.timestamp.toLocaleString();

      throw new Error(
        `Password change failed. Ensure the new password is unique and not among the last 2 used and current one  (last used on ${formattedTimestamp}).`
      );
    }
    return isMatch;
  });
  ...
  if (payload.currentPassword === payload.newPassword) {
    throw new Error(
      "Password change failed. Ensure the new password is unique and not among the last 2 used and current one"
    );
  }
